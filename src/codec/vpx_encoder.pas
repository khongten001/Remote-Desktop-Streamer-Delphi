...
{$ENDIF}
  ...
  VPX_ENCODER_ABI_VERSION = 16 + VPX_CODEC_ABI_VERSION +
    VPX_EXT_RATECTRL_ABI_VERSION + VPX_TPL_ABI_VERSION;
...
type
  ...
  vpx_codec_cx_pkt_kind = (VPX_CODEC_CX_FRAME_PKT, VPX_CODEC_STATS_PKT,
    VPX_CODEC_FPMB_STATS_PKT, VPX_CODEC_PSNR_PKT, VPX_CODEC_CUSTOM_PKT = 256);
  ...
  vpx_codec_enc_output_cx_pkt_cb_fn_t = procedure(pkt: PvpxCodecCxPkt;
  ...
  vpx_enc_pass = (VPX_RC_ONE_PASS, VPX_RC_FIRST_PASS, VPX_RC_LAST_PASS);

  // Rate control mode enumeration
  vpx_rc_mode = (VPX_VBR, VPX_CBR, VPX_CQ, VPX_Q);

  // Keyframe mode enumeration
  vpx_kf_mode = (VPX_KF_FIXED, VPX_KF_AUTO, VPX_KF_DISABLED = 0);
...
function vpx_codec_enc_init_ver(ctx: PvpxCodecCtx; iface: PvpxCodecIface;
  const cfg: PvpxCodecEncCfg; flags: vpx_codec_flags_t; ver: Integer)
...
function vpx_codec_enc_init_multi_ver(ctx: PvpxCodecCtx; iface: PvpxCodecIface;
  ...
  dsf: PvpxRational; ver: Integer): vpx_codec_err_t; cdecl; external VPX_LIB;

function vpx_codec_enc_config_default(iface: PvpxCodecIface;
  cfg: PvpxCodecEncCfg; usage: Cardinal): vpx_codec_err_t; cdecl;
  external VPX_LIB;

function vpx_codec_enc_config_set(ctx: PvpxCodecCtx; const cfg: PvpxCodecEncCfg)
  : vpx_codec_err_t; cdecl; external VPX_LIB;

function vpx_codec_get_global_headers(ctx: PvpxCodecCtx): PvpxFixedBuf; cdecl;
  external VPX_LIB;
...
function vpx_codec_encode(ctx: PvpxCodecCtx; const img: PvpxImage;
  ...
  deadline: Cardinal): vpx_codec_err_t; cdecl; external VPX_LIB;

function vpx_codec_set_cx_data_buf(ctx: PvpxCodecCtx; const buf: PvpxFixedBuf;
  pad_before: Cardinal; pad_after: Cardinal): vpx_codec_err_t; cdecl;
  external VPX_LIB;

function vpx_codec_get_cx_data(ctx: PvpxCodecCtx; iter: Pvpx_codec_iter_t)
  : PvpxCodecCxPkt; cdecl; external VPX_LIB;

function vpx_codec_get_preview_frame(ctx: PvpxCodecCtx): PvpxImage; cdecl;
  external VPX_LIB;
...
function vpx_codec_enc_init(ctx: PvpxCodecCtx; iface: PvpxCodecIface;
  const cfg: PvpxCodecEncCfg; flags: vpx_codec_flags_t): vpx_codec_err_t;
begin
  Result := vpx_codec_enc_init_ver(ctx, iface, cfg, flags,
...
function vpx_codec_enc_init_multi(ctx: PvpxCodecCtx; iface: PvpxCodecIface;
  ...
  dsf: PvpxRational): vpx_codec_err_t;
begin
  Result := vpx_codec_enc_init_multi_ver(ctx, iface, cfg, num_enc, flags, dsf,
    VPX_ENCODER_ABI_VERSION);
end;
